    ...
    <div class="bd-toc-item">
      ...
      <a class="bd-toc-link" href="../../../Design">Design</a>
        ...
        <main
          ...
          role="main"
        >
          <div class="markdown-preview"><html><head></head><body><h1>css-select <a href="https://npmjs.org/package/css-select" target="_blank" rel="noopener"><img src="http://img.shields.io/npm/v/css-select.svg" alt="NPM version"></a> <a href="http://travis-ci.org/fb55/css-select" target="_blank" rel="noopener"><img src="https://travis-ci.org/fb55/css-select.svg?branch=master" alt="Build Status"></a> <a href="https://npmjs.org/package/css-select" target="_blank" rel="noopener"><img src="https://img.shields.io/npm/dm/css-select.svg" alt="Downloads"></a> <a href="https://coveralls.io/r/fb55/css-select" target="_blank" rel="noopener"><img src="https://coveralls.io/repos/fb55/css-select/badge.svg?branch=master" alt="Coverage"></a></h1>
...
<p>In its default configuration, css-select queries the DOM structure of the <a href="https://github.com/fb55/domhandler" target="_blank" rel="noopener"><code>domhandler</code></a> module (also known as htmlparser2 DOM).
It uses <a href="https://github.com/fb55/domutils" target="_blank" rel="noopener"><code>domutils</code></a> as its default adapter over the DOM structure. See Options below for details on querying alternative DOM structures.</p>
...
<h2>Why?</h2>
<p>The traditional approach of executing CSS selectors, named left-to-right execution, is to execute every component of the selector in order, from left to right <em>(duh)</em>. The execution of the selector <code>a b</code> for example will first query for <code>a</code> elements, then search these for <code>b</code> elements. (That&apos;s the approach of eg. <a href="https://github.com/jquery/sizzle" target="_blank" rel="noopener"><code>Sizzle</code></a>, <a href="https://github.com/dperini/nwmatcher/" target="_blank" rel="noopener"><code>nwmatcher</code></a> and <a href="https://github.com/ded/qwery" target="_blank" rel="noopener"><code>qwery</code></a>.)</p>
<p>While this works, it has some downsides: Children of <code>a</code>s will be checked multiple times; first, to check if they are also <code>a</code>s, then, for every superior <code>a</code> once, if they are <code>b</code>s. Using <a href="http://en.wikipedia.org/wiki/Big_O_notation" target="_blank" rel="noopener">Big O notation</a>, that would be <code>O(n^(k+1))</code>, where <code>k</code> is the number of descendant selectors (that&apos;s the space in the example above).</p>
...
<h2>API</h2>
<pre class="hljs"><code><span class="hljs-keyword">const</span> CSSselect = <span class="hljs-built_in">require</span>(<span class="hljs-string">&quot;css-select&quot;</span>);
</code></pre>
...
<li><code>adapter</code>: The adapter to use when interacting with the backing DOM structure. By default it uses <a href="https://github.com/fb55/domutils" target="_blank" rel="noopener"><code>domutils</code></a>.</li>
</ul>
<h4>Custom Adapters</h4>
<p>A custom adapter must implement the following functions:</p>
<pre class="hljs"><code>isTag, existsOne, getAttributeValue, getChildren, getName, getParent,
...
see the <a href="https://github.com/ericelliott/rtype" target="_blank" rel="noopener"><code>rtype</code></a> or
<a href="https://www.typescriptlang.org/" target="_blank" rel="noopener"><code>TypeScript</code></a> docs, as it is very similar to
both of those. You may also want to look at
-<a href="https://github.com/fb55/domutils" target="_blank" rel="noopener"><code>domutils</code></a> to see the default
-implementation, or at
-<a href="https://github.com/nrkn/css-select-browser-adapter/blob/master/index.js" target="_blank" rel="noopener"><code>css-select-browser-adapter</code></a>
...
<h2><code>css-select</code> for enterprise</h2>
<p>Available as part of the Tidelift Subscription</p>
<p>The maintainers of <code>css-select</code> and thousands of other packages are working with Tidelift to deliver commercial support and maintenance for the open source dependencies you use to build your applications. Save time, reduce risk, and improve code health, while paying the maintainers of the exact dependencies you use. <a href="https://tidelift.com/subscription/pkg/npm-css-select?utm_source=npm-css-select&amp;utm_medium=referral&amp;utm_campaign=enterprise&amp;utm_term=repo" target="_blank" rel="noopener">Learn more.</a></p>
</body></html></div>
        </main>
      </div>
    </div>
